...
// 한진 원클릭 대량접수 실제 양식 헤더(순서 = 출력 순서). 실제 양식과 동일해야 함.
var HANJIN_COLS = [
  '받으시는 분',
  '받으시는 분 전화',
  '받는분담당자',
...
// 매핑 규칙: from=네이버값 그대로, const=고정값, fn=계산. (매핑 없는 열은 빈칸)
var MAPPING = {
  '받으시는 분':      { from: 'receiver' },
  '받으시는 분 전화': { fn: 'phone' },
  '받는분핸드폰':     { fn: 'phone' },
  '받는분우편번호':   { fn: 'zip' },
  '받는분총주소':     { from: 'address' },
  '수량':             { from: 'quantity' },
  '품목명':           { fn: 'itemName' },
  '운임Type':         { const: '신용' },   // 신용(계약)/선불/착불
  '출고번호':         { from: 'orderNo' },  // 네이버 주문번호(추적용)
  '특기사항':         { from: 'deliveryMemo' }
};
